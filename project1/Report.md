...
In this function, we use `cv::GaussianBlur` to apply a blur effect to the entire source image and store the result in a temporary blurred image. This creates a smooth background that will later be combined with the original image.

At the same time, we perform face detection on the original source image by calling the face detection function. 

Finally, we create a mask for the detected face region and copy only the blurred pixels outside the face area into the destination image. As a result, the face remains sharp while everything outside the face is blurred.
**Original:**
